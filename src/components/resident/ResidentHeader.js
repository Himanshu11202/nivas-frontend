      ...
      <div className="h-full px-4 lg:px-8 flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="hidden md:block">
            <h1 className="text-lg font-semibold text-slate-800">Resident Portal</h1>
            <p className="text-xs text-slate-500">Welcome back, {user?.name?.split(' ')[0] || 'Resident'}</p>
          </div>
        </div>
          ...
          <div className="relative">
            ...
            >
              ...
              <div className="hidden sm:block text-left">
                <p className="text-sm font-medium text-slate-700">{user?.name || 'Resident'}</p>
                <p className="text-xs text-slate-500">Flat {user?.flatNumber || 'N/A'}</p>
              </div>
              ...
              <>
                <div className="absolute right-0 top-full mt-2 w-64 bg-white rounded-xl shadow-lg border border-slate-100 py-2 z-50">
                  <div className="px-4 py-3 border-b border-slate-100">
                    ...
                    <p className="text-xs text-slate-400 mt-1">Flat {user?.flatNumber}</p>
                  </div>
